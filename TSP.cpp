...
void createInstance() {
	...
	for (int i = 0; i < numberOfNodes; i++) {
		destination << i + 1 << " " << rand() % 100 + 1 << " " << rand() % 100 + 1 << endl;
	}
...
double ACO(double(&graph)[MAX_SIZE][MAX_SIZE]) {
	...
	double alpha = 1, ro = 0.1, delta = 0.1, result = 1000000, iterResult, den, probabilitiesSum,
	...
	//for (int i = 0; i < 10; i++) zapis << bestCycle[i] << " ";
	//zapis.close();
...
int main() {
	...
	ofstream zap;
	zap.open("test.txt");
	for (double k = 0.5; k < 1; k = k + 0.1) {
		for (int i = 0; i < 5; i++) {
			q0 = k;
			xd = ACO(graph);
			cout << q0 << " " << beta << " " << fi << " " << xd << endl;
			zap << q0 <<" " << beta<< " " << fi << " "<< xd << endl;
		}
	}
	q0 = 0.9;
	for (int k = 2; k < 7; k++) {
	...
	beta = 5;
	for (double k = 0.74; k < 0.99; k = k + 0.06) {
		for (int i = 0; i < 5; i++) {
			fi = k;
			xd = ACO(graph);
			cout << q0 << " " << beta << " " << fi << " " << xd << endl;
